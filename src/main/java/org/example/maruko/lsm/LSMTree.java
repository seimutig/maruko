...
public class LSMTree {
     ...
     * Now properly handles tombstone records during compaction
     */
    private Map<String, MergedRecord> performMinHeapMerge(List<LSMFile> inputFiles) throws IOException {
        ...
        for (int i = 0; i < inputFiles.size(); i++) {
            ...
            // Read actual records from each file using real Parquet reader
            FileIterator iter = new FileIterator(entry, entry.getSequenceNumber(), i);
        ...
        Map<String, Long> deletedKeys = new HashMap<>(); // Track deleted keys with sequence numbers
        ...
        while (!heap.isEmpty()) {
            ...
            Map<String, Object> currentRecord = current.getRecord();
            Object isDeleted = currentRecord.get("_deleted");
            Long currentSequence = current.getSequenceNumber();
            
            if (isDeleted instanceof Boolean && (Boolean) isDeleted) {
                // This is a tombstone record, track the deleted key
                Long existingDeleteSeq = deletedKeys.get(currentKey);
                if (existingDeleteSeq == null || currentSequence > existingDeleteSeq) {
                    deletedKeys.put(currentKey, currentSequence);
                }
                System.out.println("Compaction: Tracking tombstone for key " + currentKey + " with sequence " + currentSequence);
            }
            ...
            if (!currentKey.equals(lastProcessedKey)) {
                // Check if this key has been deleted with a newer tombstone
                Long deleteSequence = deletedKeys.get(currentKey);
                boolean isDeletedByTombstone = (deleteSequence != null && currentSequence <= deleteSequence);
                
                if (!isDeletedByTombstone) {
                    ...
                    if (existing == null || currentSequence > existing.getSequenceNumber()) {
                        deduplicatedRecords.put(currentKey, current);
                    }
                } else {
                    System.out.println("Compaction: Skipping record for key " + currentKey + " due to newer tombstone");
                }
            }
        ...
        System.out.println("Compaction merge completed: " + deduplicatedRecords.size() + " records retained, " + deletedKeys.size() + " keys marked as deleted");
    ...
    private class FileIterator {
        ...
        private List<Map<String, Object>> records;  // Keep reference to the records to allow cleanup
        
        public FileIterator(ManifestEntry entry, long sequenceNumber, int fileIndex) throws IOException {
            // Read actual records from the file
            this.records = readActualRecords(entry);
    ...
    private static class MergedRecord {
        ...
        public String getPrimaryKey() {
            // Extract primary key from record using the helper method
            return getPrimaryKeyFromRecord(record);
        }
    }
    
    /**
     * Get records iterator for a ManifestEntry by reading from the actual file
     * This creates an iterator that can read records one by one from the physical file
     */
    private Iterator<Map<String, Object>> createFileIterator(ManifestEntry entry) throws IOException {
        // Read all records from the actual file
        List<Map<String, Object>> allRecords = readActualRecords(entry);
        
        // Return an iterator for these records
    ...
    /** 
     * Read actual records from a file using the FileStore's Parquet reader
     */
    private List<Map<String, Object>> readActualRecords(ManifestEntry entry) throws IOException {
        List<Map<String, Object>> records = new ArrayList<>();
        
        // Use the FileStore's SimpleMapParquetReader to read the actual Parquet file
        try {
            ...
            System.out.println("Successfully read " + records.size() + " records from file: " + entry.getFilePath());
            return records;
            
        } catch (Exception e) {
            System.err.println("Error reading actual records from file: " + entry.getFilePath() + ", error: " + e.getMessage());
            e.printStackTrace();
    ...
    /** 
     * Helper method to get the primary key from a record
     */
    private static String getPrimaryKeyFromRecord(Map<String, Object> record) {
        // Try to extract primary key from common key field names
        // This is a simplified approach - in practice, you'd use the actual primary key fields
        if (record.containsKey("id")) {
            return record.get("id").toString();
        } else if (record.containsKey("cardid")) {
            return record.get("cardid").toString();
        } else if (record.containsKey("name")) {
            return record.get("name").toString();
        } else {
            // If no standard key found, create a composite key from all values
